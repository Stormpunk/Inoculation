...
public class VineBranch {
    ...
    public List<VinePoint> points = new List<VinePoint>();
    ...
    public VinePoint CreateFromGameObject(GameObject obj, float thickness)
    {
        if(obj == null)
        {
            Debug.LogError("No Objecrt");
            return null;
        }
        Vector3 origin = obj.transform.position;
        Vector3 normal = obj.transform.up;
        return new VinePoint(origin, normal, thickness);
    }
    ...
    void AssignMaterials()
    {
        if (tree.planter.vineTopMat != null && tree.planter.vineBottomMat != null)
        {
            ...
            //this doesn't fit the growing shader
            meshRenderer.sharedMaterial =  tree.planter.vineTopMat;
            meshRenderer.sharedMaterials[0].mainTextureScale = new Vector3(1, textureTiling);
            //meshRenderer.sharedMaterials[1].mainTextureScale = new Vector3(1, textureTiling);

        } 
        /*if (tree.planter.vineTopMat != null && tree.planter.vineBottomMat != null)
        {
            Material singleMaterial = tree.planter.vineTopMat;  // You can choose vineTopMat or vineBottomMat

            meshRenderer.sharedMaterial = singleMaterial;
            meshRenderer.sharedMaterial.mainTextureScale = new Vector2(1, textureTiling);
        } */
    }
